...
int NSegFault=61440000;  ///61440000 dovrebbe essere il massimo assoluto
...
InRange=CAENDPP_InputRange_0_6Vpp; //for detector OPD
...
if ((ret = CAENDPP_SetInputRange(handledpp, 1, InRange)) != 0) {
	...
	{	
					...
					if(datazero==false){data=Events[ch][ev].Energy;datazero=true;}
                    			...
                    			if(write_coordinates==true && x<Xn && (y%2==0)) caso=2;//scrive le posizione andando da sinistra a destra
					if(write_coordinates==true && (x==Xn)) caso=3;//gira 
					if(write_coordinates==true && x<Xn && (y%2!=0)) caso=4;//scrive le posizione andando da destra a sinistra
