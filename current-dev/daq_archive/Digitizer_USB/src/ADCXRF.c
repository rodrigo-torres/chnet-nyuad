...
int main(int argc, char *argv[]) {
    key_t key_cmd = 6900, key = 7000, key2 = 7200, key4 = 7400, key_rate = 7500;
    key_t key5 = 8000;
    ...
    float* shared_memory5 = assignSHM(key5, 4096, 5);
...
*/
    ...
    float scan_params[TOTAL_SCAN_PARAMS];
    for (int i = 0; i < TOTAL_SCAN_PARAMS; i++)
    {
    	scan_params[i] = *(shared_memory5 + i);
    }

    int PositionX_adc = (int)scan_params[0];
    int PositionY_adc = (int)scan_params[1];
    ...
    while (*(shared_memory_cmd+70) != 0) { // While acquisition is active
        ...
        if (*(shared_memory2+3) == 1 && check) { // Handles changes in scan lines
            *(shared_memory2+10+datum_total)= PositionX_adc+codePosX; datum_total++;
            *(shared_memory2+10+datum_total)= PositionY_adc+codePosY; datum_total++; x++;
        ...
        if (write_coordinates) {
            ...
            case 1:
                ...
                *(shared_memory2+10+datum_total) = PositionX_adc+codePosX; datum_total++;
                *(shared_memory2+10+datum_total) = PositionY_adc+codePosY; datum_total++;
            ...
            for (ch = 0; ch < MaxNChannels; ch++) {
                ...
                if (AcqRun == 1) {
                    for (int ev = 0; ev < NumEvents[ch]; ev++) {
                        ...
                        if (data > 0 && data < 16384  && datum_total < NSegFault)          caso=4; // Writes the energies
                        ...
                        case 4:
                            if (ch == 0) {
                                *(shared_memory2+10+datum_total) = data+codeDetA; datum_total++;
                                *(shared_memory2+5) = ++eventi;
                                *(shared_memory+100+data) += 1;
                                *(shared_memory+40000+data) += 1;
                            }
                            if (ch == 1) {
                               *(shared_memory2+10+datum_total) = data+codeDetB; datum_total++;
                               *(shared_memory2+5) = ++eventi;
