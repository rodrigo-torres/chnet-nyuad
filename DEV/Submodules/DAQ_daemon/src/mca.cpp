...
CAENLibraryHandle::Handle CAENLibraryHandle::GetBoardHandle(MCABoardConfiguration const & config) {
  ...
  MapEnumValues(config.conn.type, conn_params.LinkType);
  conn_params.LinkNum         = config.conn.device_number;
  conn_params.ConetNode       = config.conn.daisy_chain_node;
  conn_params.VMEBaseAddress  = config.conn.vme_base_address;
