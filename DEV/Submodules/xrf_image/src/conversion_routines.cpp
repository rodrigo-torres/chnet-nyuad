...
FileConverter::FileConverter(std::string const & filename) : error{},
 filename_{filename}, data_file_{}
{
...
auto FileConverter::IsHyperCube(std::ifstream & file) -> bool
{
  std::string line;
  ...
  // string
  line = doc.child("Format").child_value();
...
auto FileConverter::OptimizedSingleDetectorFormatToHypercube() -> void
{
  ...
  auto output_filename{filename_};
  output_filename.append(".hyperc");
