...
UPDATE(uRecordLength) { 
	...
	if (_stricmp(UPDATE_SECTION,"COMMON")==0) {
		int i;
		for (i=0; i<MAX_CH; i++) dig->recordLength[i] = atoi(UPDATE_VALUE);
	} else {
		dig->recordLength[atoi(UPDATE_SECTION)] = atoi(UPDATE_VALUE);
	}
	UPDATE_RESTART();
}
...
UPDATE(uPreTrigger) {
	...
	if (_stricmp(UPDATE_SECTION,"COMMON")==0) {
		int i=0;
		for (i=0; i<MAX_CH; i++) dig->preTrigger[i] = atoi(UPDATE_VALUE)/dig->tsampl[i];
	} else {
		dig->preTrigger[atoi(UPDATE_SECTION)] = atoi(UPDATE_VALUE)/dig->tsampl[atoi(UPDATE_SECTION)];
	}
	UPDATE_RESTART();
}
...
UPDATE(uShortGate) {
	...
	if (_stricmp(UPDATE_SECTION,"COMMON")==0) {
		int i=0;
		for (i=0; i<MAX_CH; i++) dig->dppParams.sgate[i] = atoi(UPDATE_VALUE)/dig->tsampl[i];
	} else {
		dig->dppParams.sgate[atoi(UPDATE_SECTION)] = atoi(UPDATE_VALUE)/dig->tsampl[atoi(UPDATE_SECTION)];
	}
	UPDATE_RESTART();
}
...
UPDATE(uLongGate) {
	...
	if (_stricmp(UPDATE_SECTION,"COMMON")==0) {
		int i=0;
		for (i=0; i<MAX_CH; i++) dig->dppParams.lgate[i] = atoi(UPDATE_VALUE)/dig->tsampl[i];
	} else {
		dig->dppParams.lgate[atoi(UPDATE_SECTION)] = atoi(UPDATE_VALUE)/dig->tsampl[atoi(UPDATE_SECTION)];
	}
	UPDATE_RESTART();
}
...
UPDATE(uTimeValAcqWin) {
    // NOTE: the tvaw follows the motherboard clock period (8ns), so we need to divide by 8
	Digitizer* dig = UPDATE_GET_DIG();
    int val = atoi(UPDATE_VALUE)>>3;

	if (_stricmp(UPDATE_SECTION,"COMMON")==0) {
		int i;
		for (i=0; i<MAX_CH; i++)
			dig->dppParams.tvaw[i] = val;
	} else {
		dig->dppParams.tvaw[atoi(UPDATE_SECTION)] = val;
	}
	UPDATE_RESTART();
}
...
UPDATE(uVirtualProbeA) {
    //userMsg("UPDATE_VALUE: %s, atoi(UPDATE_VALUE): %d\n", UPDATE_VALUE, atoi(UPDATE_VALUE));
    ...
    if(dump) {
        ...
        for(i=0; i<MAX_CH; i++) {
            if (checkChannelEnabled(av->dig, i)) {
                ...
                dppdescr.DPPType = c_DPPType_PSD;
                dppdescr.TSType = TYPE_UINT32;
                dppdescr.ENType = TYPE_INT32;
...
CONFIGURATION_PARAMETERS (
	...
	CHANNEL_PARAMETER(	uRecordLength,		"RECORD_LENGTH",		"1024"				);
	...
	CHANNEL_PARAMETER(  uChargeSens,		"PSD_SEL_CHARGE_SENS",	"0"					);
	CHANNEL_PARAMETER(  uShortGate,			"PSD_SHORT_GATE",		"40"				);
	CHANNEL_PARAMETER(  uLongGate,			"PSD_LONG_GATE",			"80"				);
	CHANNEL_PARAMETER(  uPreGate,			"PSD_PRE_GATE",			"40"				);
	COMMON_PARAMETER(	uBaselineThreshold, "PSD_BL_THRESHOLD",		"12"				);
	COMMON_PARAMETER(	uBaselineTimeout,	"PSD_BL_TIMEOUT",		"40"				);
	CHANNEL_PARAMETER(	uTimeValAcqWin,		"TIME_VAL_ACQ_WIN",		"80"				);
	COMMON_PARAMETER(	uPURGap,			"PUR_GAP",				"80"				);
	COMMON_PARAMETER(   uPURMode,			"PUR_MODE",				"DETECT"			);
	CHANNEL_PARAMETER(	uDppOptBaseline,	"PSD_SEL_BASELINE",		"0"					);
