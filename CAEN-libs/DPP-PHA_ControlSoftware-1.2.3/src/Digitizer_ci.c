...
#define CI_GATEDSTART_MASK 0x2

#define SENDFLUSH_CI(HANDLE) CAEN_DGTZ_WriteRegister(HANDLE, 0x803C, 0) // HACK, is ok this reg for flush?

INIT_C_LOGGER("DppRunnerLog.txt", "Digitizer_ci.c");
...
static int programDigitizer(Digitizer* dig) {
    ...
    if (CAEN_DGTZ_ClearData(dig->boardHandle)) ee = RESET;
    //sleep_(100); // HACK
    ...
    //if (CAEN_DGTZ_SetRunSynchronizationMode(handle, CAEN_DGTZ_RUN_SYNC_Disabled)) ee = SYNCMODE;
    ...
    for(i=0; i<dig->numChannels; i++) {
		if (dig->enableMask & (1<<i)) {
            preTrg = max(preTrg, dig->dppParams.pgate[i]); // set PreTrg to the max of pregates
    ...
    if (dig->acquisitionMode == CAEN_DGTZ_DPP_ACQ_MODE_List)
        preTrg += PRETRG_PREGATE_MINDIFF_CI; // In list mode, the pretrigger must be forced to be the smallest possible (max_preGate+mindiff) 
    else
        preTrg = dig->preTrigger;
    ret |= CAEN_DGTZ_SetDPPPreTriggerSize(dig->boardHandle, -1, preTrg);
...
int initDigitizer(Digitizer* dig, int reopen) {
	...
	if (!dig->boardOpen || reopen) {
		...
		sscanf(dig->boardInfo->AMC_FirmwareRel, "%d.%d", &majorNumber,&minorNumber);
		if (majorNumber != 130 || minorNumber < 16) {
			userMsg("This digitizer doesn't have a DPP-CI version 2 firmware (major %d.%d)\n",majorNumber,minorNumber);
        ...
        if(FW_MIN<CI_PROBES_VER_2_AMC_FW_REL-1)
            sprintf(amcver, "AMCREL%d", 0);
        else
            sprintf(amcver, "AMCREL%d", CI_PROBES_VER_2_AMC_FW_REL);
...
int stopAcquisition(Digitizer* dig) {
	if (dig->acqStatus != ACQSTATUS_STOPPED) {
        ...
        if(dig->acqStatus == ACQSTATUS_RUNNING) {
            ...
            if((ret = SENDFLUSH_CI(dig->boardHandle))!=0) // Sent flush command
                return ret;
...
int getDecodedWaveforms(Digitizer* dig, CAEN_DGTZ_DPP_CI_Event_t* event, void* eventData[6], int* dataSize) {
	int res;
	if ((res = CAEN_DGTZ_DecodeDPPWaveforms(dig->boardHandle, event, dig->waveforms)) != 0)
		return res;
