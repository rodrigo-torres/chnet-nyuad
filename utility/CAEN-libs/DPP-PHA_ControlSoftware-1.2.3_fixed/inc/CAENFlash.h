...
typedef enum {
/* Error Types */
 CAENFlash_Success		      =  0,     /* Operation completed successfully              	*/
 CAENFlash_GenericError       = -1,     /* Generic error                        			*/
 CAENFlash_CAENCommError      = -2,     /* CAENComm function call error        			*/
 CAENFlash_UnsupportedRegSize = -3,     /* Unsupported Register Size from parameter file	*/
 CAENFlash_FileAccessError    = -4      /* File access	error */
} CAENFlash_ErrorCode;


int32_t ReadFlashPage (CAENFlashAccess *Flash, uint8_t *data, int pagenum);
int32_t ReadLicense(int32_t handle, char *licenseString);
