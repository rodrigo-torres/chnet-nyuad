...
/////////////////// SHARED MEMORY INITIALIZATION
                                  *(shared_memory4)=40;        // Trigger Threshold                                   
                                  *(shared_memory4+1)=0;       // Input range digitalizzatore (0 -> 0.6V // 1 -> 1.4V // 2 -> 3.7V // 3 -> 9.5V                                 
                                  *(shared_memory4+2)=800;     // Trapezoid Rise Time (ns) 
                                  *(shared_memory4+3)=2500;    // Trapezoid Flat Top  (ns)
                                  *(shared_memory4+4)=10000;   // Decay Time Constant (ns) HACK-FPEP the one expected from fitting algorithm?
                                  *(shared_memory4+5)=1500;    // Peaking delay  (ns)
                                  *(shared_memory4+6)=1;       // Trigger Filter smoothing factor  
                                  *(shared_memory4+7)=0;       // OFFSET
                                  *(shared_memory4+8)=1200;    // Trigger Hold Off  
                                  *(shared_memory4+9)=2;     // BaseLine Mean 
                                  *(shared_memory4+10)=1;      // peak mean (numero punti su cui mediare per peaking del trapezio)  
                                  *(shared_memory4+11)=2700;   // peak holdoff (min intervallo tra 2 trapezi...minimo=k+m)  
                                  *(shared_memory4+12)=1;      // Trapezoid Gain  
                                  *(shared_memory4+13)=100;    // NOT USED: Delay(b) Input Signal Rise time (ns)...sarebbe delay (b)
                                  *(shared_memory4+14)=1;     // NOT USED: Energy Normalization Factor
