...
int ProgramDigitizer(int handle, DigitizerParams_t Params, CAEN_DGTZ_DPP_PHA_Params_t DPPParams)
{
    ...
    for(i=0; i<MaxNChannels; i++) {
        if (Params.ChannelMask & (1<<i)) {
	      ...
	      ret |= CAEN_DGTZ_SetChannelDCOffset(handle, i, 0xB333); //for emulator and amptek_PA210_50mm2_ch1_Gain6.7
	    //ret |= CAEN_DGTZ_SetChannelDCOffset(handle, i, 0xEF00); //for emulator with DT5780 modified
...
/*****parameters for XR100-SDD with PA210 50mm2******_ch1_Gain6.6*/
  	...
  	/*****parameters for XR100-SDD with PA210 50mm2******/
/*
					...
					if(write_coordinates==true && x<(Xn-1) && (y%2!=0)) caso=4;//scrive le posizione andando da destra a sinistra
     					...
     					switch (caso)
     					{
							...
							*(shared_memory2+10+n)=data;
							...
							//printf("*(shared_memory+100+data:%d, data:%d\n", *(shared_memory+100+data), data);
							caso=10;
        						break;
        					}///chiude case 1
