...
bool ScanWithNoData=false; extern bool QuitProgram_bool; int ReadOutErrors=0;
...
uint32_t chch = 0;  //channel number (0, 1)
...
int ProgramDigitizer(int handle, DigitizerParams_t Params, CAEN_DGTZ_DPP_PHA_Params_t DPPParams)
{
    ...
    for(i=0; i<MaxNChannels; i++) {
	   ...
	   // ret |= CAEN_DGTZ_SetChannelDCOffset(handle, i, 0xB333); //for emulator
	    ret |= CAEN_DGTZ_SetChannelDCOffset(handle, i, 0xB333); //for amptek_PA210_50mm2_ch1_Gain6.7 // 0xD1EA old 0xA8F5 0x599A
		// ret |= CAEN_DGTZ_SetChannelDCOffset(handle, i, 0xB70A); //for detector opd
            //ret |= CAEN_DGTZ_SetChannelDCOffset(handle, i, 0xEB85);   //era 0x8000 cioè la metà di 0xFFFF (l'offset DC è in 16 bit)
                                                                      ...
                                                                      ////0xEF00); //for emulator with DT5780 modified
...
/*****parameters for XR100-SDD with PA210 50mm2******_ch1_Gain6.6*/
  	...
  	/*****parameters for XR100-SDD with PA210 50mm2******_ch0_Gain14.33/

         
            DPPParams[b].thr[ch] =  1000;     //Trigger Threshold 

           //previously set to 10000
            DPPParams[b].k[ch] =  1000;    //Trapezoid Rise Time (ns)
            DPPParams[b].m[ch] = 1000;      //Trapezoid Flat Top  (ns)
           //previously set to 300000
            DPPParams[b].M[ch] = 7200;    //Decay Time Constant (ns) HACK-FPEP the one expected from fitting algorithm? 
            DPPParams[b].ftd[ch] = 600;    //Peaking delay  (ns)
            DPPParams[b].a[ch] = 1;         //Trigger Filter smoothing factor (position in the menu)
            DPPParams[b].b[ch] = 200;       //Delay(b)  // Input Signal Rise time (ns)
            DPPParams[b].trgho[ch] = 1300; //Trigger Hold Off
            DPPParams[b].nsbl[ch] = 6;      //Baseline mean del trapezio (ordine di comparsa nel menu)
            DPPParams[b].nspk[ch] = 16;      //Peak mean (numero punti su cui mediare per peaking del trapezio)
            DPPParams[b].pkho[ch] = 2500;   //Peak holdoff (ns) (min intervallo tra 2 trapezi...minimo=k+m)
            DPPParams[b].blho[ch] = 10000;    //Baseline holdoff del trapezio (ns) (intervallo dove non calcolare la baseline 0 sta per default=m+2*k)
            DPPParams[b].enf[ch] = 1;      //Energy Normalization Factor or Trapezoid Gain
            //DPPParams[b].tsampl[ch] = 10;
            DPPParams[b].dgain[ch] = 0;
	...
	/*****parameters for the emulator XR100-SDD with PA210 50mm2******/
	
	   // DPPParams[b].thr[ch] =  1000;
           // DPPParams[b].k[ch] =  300;
           ////DPPParams[b].m[ch] = 43000; 
           // DPPParams[b].M[ch] = 3400; 
           // DPPParams[b].ftd[ch] = 2400; 
           // DPPParams[b].a[ch] = 1; 
           // DPPParams[b].b[ch] = 4200; 
          //  DPPParams[b].trgho[ch] = 1300;
          //  DPPParams[b].nsbl[ch] = 16;
          //  DPPParams[b].nspk[ch] = 16;          
          //  DPPParams[b].pkho[ch] = 2500;
...
/*****parameters for the detector SDD-XR100-lungo (Direct coupling) and CH1-Dt5780 modified (Gain 12- No INrange variable)******/	
	   // DPPParams[b].thr[ch] = 115;// Trigger Threshold 
           // DPPParams[b].k[ch] =  6300;// Trapezoid Rise Time (ns)
          //  DPPParams[b].m[ch] = 4200;// Trapezoid Flat Top  (ns)
          //  DPPParams[b].M[ch] = 9100;// Decay Time Constant (ns)
          //  DPPParams[b].ftd[ch] = 2000;// Peaking delay  (ns)
          //  DPPParams[b].a[ch] = 32; // Trigger Filter smoothing factor
          //  DPPParams[b].b[ch] = 300; // Delay(b)
          //  DPPParams[b].trgho[ch] = 2000;// Trigger Hold Off
          //  DPPParams[b].nsbl[ch] = 5;// Baseline mean del trapezio (ordine di comparsa nel menu)
          //  DPPParams[b].nspk[ch] = 2;// Peak mean (ordine di comparsa nel menu)        
          //  DPPParams[b].pkho[ch] = 1100;// Peak holdoff
          //  DPPParams[b].blho[ch] = 50;// Baseline holdoff del trapezio
	  ...
	  for(ch=0; ch<MaxNChannels; ch++)
          {
            ...
            DPPParams[b].b[ch] = 100;       //Delay(b)  // Input Signal Rise time (ns)
            ...
            DPPParams[b].blho[ch] = 1000;    //Baseline holdoff del trapezio (intervallo dove non calcolare la baseline 0 sta per default=m+2*k)
            DPPParams[b].enf[ch] = 1;      //Energy Normalization Factor
            //DPPParams[b].tsampl[ch] = 10;
            DPPParams[b].dgain[ch] = 0;
            DPPParams[b].decimation[ch] = 0; 
            ...
            printf("Params read from memory:\n");
            printf("Threshold: %d\n",*(shared_memory4));
            printf("InRange: %d\n",*(shared_memory4+1));
            printf("Rise time: %d\n",*(shared_memory4+2));
            printf("Flat top: %d\n",*(shared_memory4+3));
            ...
            printf("Trigger holdoff: %d\n",*(shared_memory4+8));
            printf("Baseline mean: %d\n",*(shared_memory4+9));
            printf("Peak mean: %d\n",*(shared_memory4+10));
            printf("Peak holdoff: %d\n",*(shared_memory4+11));
          }  //endif
...
if(*(shared_memory4+15)==0) ///if input range has not been loaded from shm
	InRange=CAENDPP_InputRange_3_7Vpp;
else{  ///if input range must be loaded from shm

          if(*(shared_memory4+1)==0) InRange=CAENDPP_InputRange_0_6Vpp;
	  else 	
	  if(*(shared_memory4+1)==1) InRange=CAENDPP_InputRange_1_4Vpp;
	  else
	  if(*(shared_memory4+1)==2) InRange=CAENDPP_InputRange_3_7Vpp;
	  else
	  if(*(shared_memory4+1)==3) InRange=CAENDPP_InputRange_9_5Vpp;
}
// Set input range
...
/*
              ...
              CAEN_DGTZ_ErrorCode ret;
              ret = CAEN_DGTZ_ReadRegister(handle, 0x1088 | (chch << 8), &channel_status);
              if (((channel_status >> 2) & 0x1) == 1)  //bit 2 del registro di stato = busy flag
                 printf("Channel %d busy! at beginning of for\n", (int)chch);
...
//printf ("time 2:%lu,%lu\n", tv, millisec);
		...
		/* Read data from the boards */
			...
			//printf("oi2, ret:%d\n");
            		if (ret) 
             		{
                		ReadOutErrors++; continue; //printf("Readout Error: %d\n", ret);
                		///goto QuitProgram;    
              		}
...
//////////////////////////////////////////////////////// INSERITO /////////////////////////////////////////////////////////////////////////
            		...
            		if (BufferSize == 0 && write_coordinates){





                    			if(x<(Xn-1) && (y%2==0)) caso=2;//scrive le posizione andando da sinistra a destra
					if((x==(Xn-1))) caso=3;//gira 
					if(x<(Xn-1) && (y%2!=0)) caso=4;//scrive le posizione andando da destra a sinistra
					
                                        //printf("caso letto %d  Energia %d\n", caso,Events[ch][ev].Energy);
     					switch (caso)
     					{
...
//////////////////////////////////////////////////////////////////////////////////// end scani with no data
	...
	{	
					...
					if(datazero==false){data=Events[ch][ev].Energy;datazero=true;}
                    			...
                    			/* Energy */
                    			if(data > 10 && data < 16000  && n<NSegFault) caso=1; //if(data > 7000  && data < 16000 ) printf("%d\n",data);//Scrive le energie
...
printf("Closing Digitizer\n");
	...
	*(shared_memory2+4)=n;  
    printf("ntot_adcxrf:%d\tReadOutErrors%d\n", *(shared_memory2+4),ReadOutErrors);
...
{
QuitProgram_bool=true;                         ////////////////////// metterlo come global variable e ammazzare col get pid il processo e riavvio digitalizzatore

  printf("\n recorded events:%d\t ReadOutErrors%d\n",eventi,ReadOutErrors);
