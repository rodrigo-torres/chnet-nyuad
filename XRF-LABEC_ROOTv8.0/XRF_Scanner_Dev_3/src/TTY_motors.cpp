...
if(noKeyence_init)
{InizializzazioneKeyence();  printf("ACM assigned!!!\n"); }              ///// CALL FOR THE ACM PORT CONNECTION  (return mising!!!!!!!!!)
else
...
{
QString commentoACM="ACM"; QString commentoACM_long=""; QString NameNumber;
...
if (noKeyence_init)
{
  ...
  if(serialK<0)
    {
     printf("ERROR opening ACM port\n");//<< MYTTY_ACM<< strerror(errno);
//      exit(-1);
    }
  else{
  struct termios my_termios;
  ...
  int h=cfsetospeed(&my_termios,B115200);//9600);
  my_termios.c_oflag &= (unsigned short)(~(ONLCR | OPOST));
  my_termios.c_cflag |= CLOCAL;
  my_termios.c_lflag &= (unsigned short)(~(ICANON | ECHO | ISIG));  
  my_termios.c_cc[VMIN] = 1;
