...
//     const QString searchString("CAENUSBdrvB");
 ...
 //         if (content.contains("CAENUSBdrvB", Qt::CaseSensitive)) {module=1;}  //(searchString, Qt::CaseSensitive)) 
         if (content.contains("a3818", Qt::CaseSensitive)) {module=1;}  //(searchString, Qt::CaseSensitive)) 
        }
      }
      file2.close();

      if(module==0){cout<<"CAEN Optical Link module not found....  module_value: "<< module << '\n';}
      if(module==1){cout<<"CAEN Optical Link module already loaded....  module_value: "<< module <<'\n';;}
...
//     Q_INIT_RESOURCE(application);
      ...
      switch (module)
       {
        case(0): {system("su -c 'sysctl -w kernel.shmmax=2000000000;sysctl -w kernel.shmall=720000000; chmod 777 /dev/ttyUSB*;  chmod 777 /dev/ttyACM*; chown frao:frao /dev/ttyACM*; chown frao:frao /dev/ttyUSB*; insmod /home/modules/a3818.ko.xz;'"); break;}
