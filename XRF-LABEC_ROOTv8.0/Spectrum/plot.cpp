...
void Plot::Calibration( double constant )
{
 ...
 //qDebug()<<"calibration: costante di calibrazione"<<cal<<"offset"<<offset<<'\n';
...
// qDebug()<<"offset:costante di calibrazione"<<cal<<"offset"<<offset<<'\n';
 if(cal==1 && offset==0) {
	setAxisTitle( QwtPlot::xBottom, "Channel" );
	*(shared_memory+24)=0;
	}
...
//          int dato=0, dato1=0, dato2=0;
	     ...
	     {qDebug()<<"file cannot be opened"; return;}
		  ...
		  {case 0: //16k
 			 ...
 			 {int dato=line.toInt();  counts[iLine]=dato; *(shared_memory+100+iLine)=counts[iLine]; if(dato>=max)max=dato; break;}
  		  
		  case 1: //8k
			
 			 {int dato1=line.toInt(); *(shared_memory+100+iLine)=dato1; datotmp=datotmp+dato1; j2count++; if(j2count==2) {counts[iLineCounts]=datotmp; if(datotmp>=max)max=datotmp; datotmp=0; j2count=0; iLineCounts++;} break;}
			 
 		  case 2: //4k
			
 			 {int dato2=line.toInt(); *(shared_memory+100+iLine)=dato2; datotmp=datotmp+dato2; j4count++; if(j4count==4) {counts[iLineCounts]=datotmp; if(datotmp>=max)max=datotmp; datotmp=0; j4count=0; iLineCounts++;} break;}
			 
  		  }//end switch

                iLine++;
                }// end while
	  maxcal = lenght*step+offset;
	  mincal = offset;
          fileOpened.close();
          aperto=true;
         }//end else
...
if(step!=1 || offset!=0) EnergyOn = true; //abbiamo aperto lo spettro in energia 
 ...
 for(int h=0;h<16384;h++)
  ...
  switch (resolution)
	  {case 0: //16k
		 ...
		 if(logchecked==true && counts[k]>0.0)
		  {
...
if(autoscaleOn) { setAxisScale(QwtPlot::yLeft, min, max); 
//printf("autoscale on, max %f, min %f\n", max, min);
}
