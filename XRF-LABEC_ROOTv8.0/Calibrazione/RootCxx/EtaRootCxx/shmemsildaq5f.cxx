...
// g++ `root-config --cflags --glibs` -o mainframe shmemsildaq_version.cxx ex2aDict.cxx
// in example2a.h devono essere definite le funzioni
...
bool Calibration=false; float CalFactor=1.0; int CalValue=1; float xF;
...
MyMainFrame::MyMainFrame(const TGWindow *p,UInt_t w,UInt_t h) {  //Tutte le modifiche fatte qui dentro devono passare dal ROOTCINT
  ...
  fEcanvas = new TRootEmbeddedCanvas("Ecanvas",fMain,720,460);
  ...
  TGHorizontalFrame *hframe = new TGHorizontalFrame(fMain,720,30);
...
/* jInt=0    ShmCommandWrite           MainCommand  
  ...
  */
  ...
  if (GraphHistoSel==0)
    {Histo=true; Graph=false; GFit=false; H2D=false;}
  if (GraphHistoSel==1) 
    {Histo=false; Graph=true; GFit=false; H2D=false;}
  if (GraphHistoSel==2) 
    {Histo=false; Graph=false; GFit=true; H2D=false;}
  if (GraphHistoSel==3) 
    {Histo=false; Graph=false; GFit=false; H2D=true;}
  ...
  if(Histo)             
    {
      ...
      if(Calibration)
      {CalValue=*(shared_memory+42); CalFactor= CalValue*0.0001;
      min=int(ZoomMin*CalFactor); max=int(ZoomMax*CalFactor); amp=ZoomAmp;
      }
      else {min=ZoomMin; max=ZoomMax; amp=ZoomAmp;}
      ...
      while(! feof(fpx))
	{
	  fscanf(fpx,"%f\n",&xF); xF=xF*CalFactor;	//in origine era float
  ...
  if(H2D)             
	...
	case 1:             
	  { //if(XampH2>=50){XampH2=50;} if(YampH2>=50){YampH2=50;}
	    TH2F *f1=new TH2F("f1","AmsDefel",100,XminH2,XmaxH2,100,YminH2,YmaxH2);
	    f1->Reset();/*fflush(fpx); */  recorded_iev =0;    
	    fpx = fopen(defDaq, "r");
	    while(! feof(fpx))
	      { fscanf(fpx,"%i\t",&x); fscanf(fpx,"%i\n",&z);  		//in origine era float
	    ...
	    *(shared_memory+2)=recorded_iev; // Writing collected events in ShmMemory
	    ...
	    f1->GetXaxis()->SetTitle(xaxis);
	    f1->SetTitle(htitle); 
	    TCanvas *fCanvas = fEcanvas->GetCanvas();   gStyle->SetPalette(1);
