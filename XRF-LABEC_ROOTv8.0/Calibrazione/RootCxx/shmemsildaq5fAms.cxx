...
//      printf("ROOTGui--> Fscale %i\tAmpScale %f\n",ScaleF,AmpScale);
      ...
      if(XampH2<16){XampH2=16;} if(XampH2>256){/*printf("Xamp Overflow\n");*/ XampH2=256;}
      if(YampH2<16){YampH2=16;} if(YampH2>256){/*printf("Yamp Overflow\n");*/ YampH2=256;}
  ...
  if(H2D)             
	...
	case 0:
	  { XminH2Cal=int(XminH2*CalFactor); XmaxH2Cal=int (XmaxH2*CalFactor);
	    ...
	    fpx = fopen(defDaq, "r"); 
	    while(! feof(fpx))
	      { fscanf(fpx,"%i\t",&x); xCal=x*CalFactor; fscanf(fpx,"%i\n",&z); zCal=(1/(sqrt(z)));  
