...
%
% XRFMultidet_v11.1
% PixelCrct() function has been removed as it was not called
% SelectionElement() functions have been removed and replaced by the single function writeCompMapLimits() which uses the signalmapper to identify the sender of the signal, and assigns the corresponding value to the global variables
% The LoadElementsmapSum() function has been modified so as to produce a dialog with spinboxes as opposed to comboboxes, now the limits for the composed map are inputted manually as oppised to being loaded from an outdated txt file
% Other aesthetic modifiations for this dialog box such as a clearer indication intro and a different layout
% MapCorrection() and CutPixels()
% Also added the correction rules for the displaySumMap to pass the limits correctly for the composed map
% change the type from int to double for composed map element limits
